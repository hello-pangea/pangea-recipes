...
export type ParamSchemaFor<Path extends string> = z.ZodObject<
  Required<Record<PathParamNames<Path>, z.ZodTypeAny>>
>;
...
>(path: Path, config: Config): Contract<Path> & Config {
  return { path, ...config } as Contract<Path> & Config;
}
...
function validateParams<C extends Contract<string>>(
  contract: C,
  incoming: unknown,
): Record<string, string> | undefined {
